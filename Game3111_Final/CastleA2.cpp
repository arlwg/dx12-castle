...
private:
    ...
    void BuildMaterials();
	void BuildItem(std::string shape_type, XMMATRIX scale_matrix, XMMATRIX translate_matrix,const char* material, UINT obj_idx);
	void BuildItemRotation(std::string shape_type, XMMATRIX scale_matrix, XMMATRIX rotation_matrix,  XMMATRIX translate_matrix, const char* material, UINT obj_idx);
...
void TreeBillboardsApp::BuildBoxGeometry()
{
	...
	GeometryGenerator::MeshData hemisphere = geoGen.CreateGeosphere(2.f, 10);
	...
	GeometryGenerator::MeshData truncatedPyramid = geoGen.CreateTruncatedPyramid(1.f,1,4,10);
	GeometryGenerator::MeshData truncatedCone= geoGen.CreateTruncatedCone(1.f,1,10);
	GeometryGenerator::MeshData cuboid = geoGen.CreateCuboid(1.0f, 1.0f, 1.0f, 3);
	//
	...
	UINT truncatedConeVertexOffset = truncatedPyramidVertexOffset + (UINT)truncatedPyramid.Vertices.size();
	UINT cuboidVertexOffset = truncatedConeVertexOffset + (UINT)truncatedCone.Vertices.size();
	...
	UINT truncatedConeIndexOffset = truncatedPyramidIndexOffset + (UINT)truncatedPyramid.Indices32.size();
	UINT cuboidIndexOffset = truncatedConeIndexOffset + (UINT)truncatedCone.Indices32.size();
	...
	SubmeshGeometry cuboidSubmesh;
    cuboidSubmesh.IndexCount = (UINT)cuboid.Indices32.size();
    cuboidSubmesh.StartIndexLocation = cuboidIndexOffset;
    cuboidSubmesh.BaseVertexLocation = cuboidVertexOffset;
	//
	...
	for (size_t i = 0; i < box.Vertices.size(); ++i, ++k)
	{
		...
		//vertices[k].Color = XMFLOAT4(0.7055f, 0.7529f, 0.5129f, 1.0f);
	}
	for (size_t i = 0; i < sphere.Vertices.size(); ++i, ++k)
	{
		...
		//vertices[k].Color = XMFLOAT4(0.1213,0.5471, 0.148,1);
	}
	...
	for (size_t i = 0; i < pyramid.Vertices.size(); ++i, ++k)
    {
       ...
       //vertices[k].Color = XMFLOAT4(0.4905,0.016, 0.018,1);
    }
	...
	for (size_t i = 0; i < cuboid.Vertices.size(); ++i, ++k)
    {
        vertices[k].Pos = cuboid.Vertices[i].Position;
		vertices[k].Normal = cuboid.Vertices[i].Normal;
		vertices[k].TexC = cuboid.Vertices[i].TexC;
        //vertices[k].Color = XMFLOAT4(DirectX::Colors::DarkOrange);
    }
	...
	indices.insert(indices.end(), std::begin(cuboid.GetIndices16()), std::end(cuboid.GetIndices16()));
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(Vertex);
	const UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);
	...
	geo->DrawArgs["cuboid"] = cuboidSubmesh;
...
void TreeBillboardsApp::BuildItem(std::string shape_type, XMMATRIX scale_matrix, XMMATRIX translate_matrix, const char* material, UINT obj_idx)
{
...
void TreeBillboardsApp::BuildItemRotation(std::string shape_type, XMMATRIX scale_matrix, XMMATRIX rotation_matrix,  XMMATRIX translate_matrix, const char* material, UINT obj_idx)
{
	auto shape_render_item = std::make_unique<RenderItem>();
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	BuildItem("grid", XMMatrixScaling(1, 1, 1), XMMatrixTranslation(0.0f, -0.01f, 0.0f),"wirefence", Index++);

	//Front wall Left
	BuildItem("box", XMMatrixScaling(-18, 10.f, 1.f), XMMatrixTranslation(-12, 5, -23),"wirefence", Index++);
	//Front wall Right
	BuildItem("box", XMMatrixScaling(18, 10.f, 1.f), XMMatrixTranslation(12, 5, -23), "wirefence", Index++);
	//Front wall Top
	BuildItem("box", XMMatrixScaling(-7.f, 2.f, 1.f), XMMatrixTranslation(0, 9, -23), "wirefence", Index++);
	//Back wall
	BuildItem("box", XMMatrixScaling(-47.f, 10.f, 1.f), XMMatrixTranslation(0, 5, 23), "wirefence", Index++);
	//Left wall
	BuildItem("box", XMMatrixScaling(1.f, 10.f, -47.f), XMMatrixTranslation(-23, 5, 0), "wirefence", Index++);
	//Right wall
	BuildItem("box", XMMatrixScaling(1.f, 10.f, -47.f), XMMatrixTranslation(23, 5, 0), "wirefence", Index++);

	//Inner front wall
	BuildItem("box", XMMatrixScaling(-38.5f, 10.f, 1.f), XMMatrixTranslation(0, 5, -19), "wirefence", Index++);
	//Inner back wall
	BuildItem("box", XMMatrixScaling(-38.5f, 10.f, 1.f), XMMatrixTranslation(0, 5, 19), "wirefence", Index++);
	//Inner left wall
	BuildItem("box", XMMatrixScaling(1.f, 10.f, -38.5f), XMMatrixTranslation(-19, 5, 0), "wirefence", Index++);
	//Inner right wall
	BuildItem("box", XMMatrixScaling(1.f, 10.f, -38.5f), XMMatrixTranslation(19, 5, 0), "wirefence", Index++);

	//Pyramid
	BuildItemRotation("pyramid", XMMatrixScaling(20.f, 20.f, 20.f), XMMatrixRotationY(0.7853981634), XMMatrixTranslation(0, 9.5, 0), "wirefence", Index++);
	...
	for (int i = 0; i < 44; ++i)
	{
		...
		{
			//Wedge front
			BuildItem("wedge", XMMatrixScaling(1.f, 1.f, 1.f), XMMatrixTranslation(-21 + i, 10.5, -23), "wirefence", Index++);
			//Wedge back
			BuildItemRotation("wedge", XMMatrixScaling(1.f, 1.f, 1.f), XMMatrixRotationY(3.1415926536), XMMatrixTranslation(-21 + i, 10.5, 23), "wirefence", Index++);
			//Wedge left
			BuildItemRotation("wedge", XMMatrixScaling(1.f, 1.f, 1.f), XMMatrixRotationY(1.5707963268), XMMatrixTranslation(-23, 10.5, -21 + i), "wirefence", Index++);
			//Wedge right
			BuildItemRotation("wedge", XMMatrixScaling(1.f, 1.f, 1.f), XMMatrixRotationY(4.7123889804), XMMatrixTranslation(23, 10.5, -21 + i), "wirefence", Index++);
		}

	}

	//Outer Cylinder Front Left
	BuildItem("cylinder", XMMatrixScaling(4.f, 15.f, 4.f), XMMatrixTranslation(-23, 7.5, -23), "wirefence", Index++);
	...
	BuildItem("cylinder", XMMatrixScaling(2.f, 15.f, 2.f), XMMatrixTranslation(-19, 7.5, -19), "wirefence", Index++);
	...
	BuildItem("cone", XMMatrixScaling(4.f, 6.f, 4.f), XMMatrixTranslation(-23, 17, 23), "wirefence", Index++);
	...
	BuildItem("sphere", XMMatrixScaling(2.f, 2.f, 2.f), XMMatrixTranslation(-19, 15, -19), "wirefence", Index++);
	...
	//Front Gate
	BuildItemRotation("cuboid", XMMatrixScaling(3.f, 8.f, 0.51f), XMMatrixRotationY(3.14159), XMMatrixTranslation(0, 3.5, -23), "wirefence", Index++);
